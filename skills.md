...
### Data Connector Pattern (when widget accepts `data` as prop)

Some widgets separate rendering from data fetching entirely. In this case a third bundle — the **DataConnector** — handles all API calls.

```
src/
  components/
    {Name}/              ← pure renderer, no API calls, no fetch imports
    {Name}Configuration/ ← config panel
  data-connectors/
    {Name}Connector/
      {Name}Connector.ts  ← polls API, calls widget.update() with data prop
      index.ts            ← self-registration on window.ReactWidgets['{Name}Connector']
```

**Three separate webpack entries when using this pattern:**
```js
const COMPONENTS = {
  {Name}: './src/components/{Name}/index.ts',
  {Name}Configuration: './src/components/{Name}Configuration/index.ts',
  {Name}Connector: './src/data-connectors/{Name}Connector/index.ts',
};
```

**Connector responsibility:**
- Polls API every `refreshInterval` seconds
- Calls `window.ReactWidgets['{Name}'].update(widgetId, { ...props, data: rawApiResponse })`
- Passes **raw API response** as `data` — never pre-process or extract values in the connector
- The widget reads from `data` and handles its own parsing

**Widget responsibility when using connector:**
- Accepts `data` as prop
- No `fetch`, no `setInterval`, no `api.ts` imports in the widget bundle
- Reads value from `data` (e.g. `data?.data?.[0]?.data`)
...
### ⚠️ Required headers on EVERY API call to connector.iosense.io

Every request — without exception — must include these four headers:

```typescript
headers: {
  'Authorization': `Bearer ${token}`,
  'Content-Type': 'application/json',
  'organisation': localStorage.getItem('organisation') ?? '',  // ← REQUIRED
  'ngsw-bypass': 'true',   // ← REQUIRED — bypasses Angular service worker; missing this causes requests to hang in pending state
}
```

**`ngsw-bypass: 'true'` is mandatory.** Without it, requests to `connector.iosense.io` get intercepted by the Angular service worker and hang in a pending state indefinitely. This is a silent failure — no error thrown, request just never completes.

**`organisation` header is mandatory.** Missing it causes auth failures even with a valid Bearer token.

### API selection by operator

Do NOT use `getLastDPsofDevicesAndSensorProcessed` for operators other than LastDP. Use `getWidgetData` for all operators:

| Operator in UI | Store in dataConfig.operator | Sent to API |
|---------------|------------------------------|-------------|
| Mean | `'mean'` | `'mean'` |
| Sum | `'sum'` | `'sum'` |
| Min | `'min'` | `'min'` |
...
| Run Hours | `'runhours'` | `'runhours'` |
| Last DP | `'lastDP'` | `'lastDP'` |

**Store API-ready operator values in `dataConfig.operator`** — not display labels. The connector/widget passes `dataConfig.operator` directly to the API with zero mapping. Map display labels → API values once, in the configuration panel only.

```typescript
// In GaugeConfiguration (or any config panel) THESE OPERATORS CAN BE USED IN DATA SOURCE CONFIGURATION FOR OPERATOR FIELDS ALSO 
const OPERATORS = [
  ...
  { label: 'Last DP',     value: 'lastDP' },
];
// Store selected option's `value` directly into dataConfig.operator
```

### `type` field in getWidgetData body

The `type` field in the request body is **widget-type-specific**, not generic. Do not guess — confirm the correct value per widget:

| Widget | type value |
|--------|-----------|
| CombineBarLineChart | `'combinedBarLineChartV2'` |
| Gauge / single-value | `'pieChart'` |

If unsure, check with the user before sending — a wrong `type` value silently returns wrong data.

### getWidgetData response shape — two variants

**Time-series widgets** (e.g. CombineBarLineChart) — nested by timeFrame:
```
response.data.data[timeFrame][bucket][].data  → value
response.data.labelConfig[timeFrame][bucket]  → human label ("30 Mar")
```

**Single-value widgets** (e.g. Gauge, LastDP) — flat array:
```
response.data[]  →  [{ type, devID, sensor, operator, key, data: number }]
// Read value as: response.data[0].data
// NOT response.data.data — no double nesting for flat responses
```

**Always verify which shape you're getting** — log the raw response before writing parsing logic. Do not assume nesting based on the MCP docs alone.
...
### getWidgetData — time-series formatting (CombineBarLineChart and similar)
```
...
### Timezone in requests

Always read timezone from `config?.time?.timezone ?? 'Asia/Calcutta'`. Populate timezone options using `Intl.supportedValuesOf('timeZone')` — no hardcoded list needed.
...
## 8. No Hardcoded Keys, Values, or Expressions — Everything Dynamic

**Widget code must never contain hardcoded identifiers, magic strings, fixed lists, or baked-in business logic.** Every behavior must be driven by config, props, or API responses. This is non-negotiable.

### What "hardcoded" means

Any literal value in widget or connector code that should instead come from `config`, `dataConfig`, API response, or user input:

| ❌ Hardcoded (never do) | ✅ Dynamic (always do) |
|------------------------|----------------------|
| `devID: 'JWEM_A13'` | `devID: config.charts[n].dataConfig.devID` |
| `sensor: 'D43'` | `sensor: config.charts[n].dataConfig.sensor` |
| `clusterID: '68ce7a856f2acb90621f2e69'` | `clusterID: config.charts[n].dataConfig.clusterID` |
| `operator: 'sum'` as a fixed default in widget logic | `operator: config.charts[n].dataConfig.operator ?? 'lastDP'` — read from config, fallback only as safe default |
| `timezone: 'Asia/Calcutta'` as the only timezone | `timezone: config?.time?.timezone ?? 'Asia/Calcutta'` — configurable, fallback is a default |
| `if (sensor === 'D0') showAlert()` | Condition driven by `config.charts[n]` comparison rules — widget evaluates generically |
| `type: 'combinedBarLineChartV2'` fixed in widget code | `type` determined by widget identity or config — never a random string pasted into fetch logic |
| `key: 'temperature'` in API call body | `key` generated from config (e.g. `config.charts[n].dataConfig.key` or derived from chart index) |
| `unit: '°C'` rendered in widget | `unit: config.charts[n].dataConfig.unit` or read from `device.unitSelected[sensor]` |
| `threshold > 50` as a fixed comparison | Threshold from `config.charts[n].fixedValue` or similar config field |
| Fixed color `'#ff0000'` for alert state | Color from `config.style` or `config.charts[n].color` |
| `pollInterval = 5000` baked into widget | `pollInterval = (config?.pollIntervalSeconds ?? 30) * 1000` |
| Static list of 3 devices in a dropdown | Dynamic list from `findUserDevices` API call |
| `if (config.charts.length === 3)` — logic tied to specific count | Iterate `config.charts` generically — `config.charts.forEach(...)` or `config.charts.map(...)` |

### Rules

1. **All identifiers come from config or API responses.** Device IDs, sensor IDs, cluster IDs, flow IDs, operator values, keys, units — none of these should ever appear as string literals in widget or connector code (except as fallback defaults in `?? 'fallback'` expressions).

2. **All thresholds, conditions, and comparisons are config-driven.** If the widget evaluates "is value > X?", then X must come from config — not from a constant in the source code.

3. **All visual properties are config-driven.** Colors, labels, units, titles, border styles — if the user can see it, the user should be able to configure it. Never hardcode visual values that should be customizable.

4. **All lists and options are dynamically populated.** Device lists from `findUserDevices`, sensor lists from `getDeviceSpecificMetadata`, cluster lists from `fetchClustersPaginated`, timezone lists from `Intl.supportedValuesOf('timeZone')`. No static arrays of options that should come from APIs or platform utilities.

5. **Widget logic must be generic across any number of charts/events/series.** Never write code that assumes a specific count of items. Use `.map()`, `.forEach()`, `.filter()`, `.find()` over `config.charts`. Never access `config.charts[0]` without checking existence first.

6. **Connector/widget must not embed widget-type assumptions.** The `type` field in `getWidgetData` is the only place where widget identity matters — and even that should be a constant defined once at the top of the file, not scattered through the code.

7. **Fallback defaults are acceptable — hardcoded business values are not.** `config?.pollIntervalSeconds ?? 30` is fine (safe default). `const ALERT_DEVICE = 'PTH_1026'` is not (hardcoded business value).

### How to audit

Before submitting any widget code, search for:
- Any `devID`, `sensor`, `clusterID`, `flowID` string literals (excluding type definitions)
- Any numeric comparisons not sourced from config (`> 50`, `=== 100`, `< 0`)
- Any color hex codes not sourced from config or CSS variables
- Any string literals used as display text that should be configurable labels
- Any fixed-length array assumptions (`charts[0]`, `charts[1]`, `charts[2]` without generic iteration)

If you find any, refactor to read from config, props, or API response.

---

## 9. Common Pitfalls — Never Do These
...
| `response.data.data` without checking nesting | Always log the raw response first — shape varies by widget type |
...
| Missing `ngsw-bypass` or `organisation` headers | Every call to connector.iosense.io needs both — missing either causes silent failures or auth errors |
| Using `getLastDPsofDevicesAndSensorProcessed` for non-LastDP operators | Use `getWidgetData` for all operators — only LastDP works with the last-datapoints API |
| Sending wrong `type` value in getWidgetData body | Confirm correct value per widget — `'combinedBarLineChartV2'` for charts, `'pieChart'` for gauge/single-value |
| Horizontal overflow in config panel | Every layout div needs `width: 100%; box-sizing: border-box;`. Tab content container needs `overflow-x: hidden` |
| Config panel content overflows horizontally | Add `overflow-x: hidden; overflow-y: auto` to the tab content wrapper |
| API calls inside the widget bundle when using connector pattern | Widget must be a pure renderer — zero fetch, zero interval, zero api.ts imports |
| `DateSelectorButton` prop named `isSelected` | Correct prop is `isActive` — `<DateSelectorButton isActive={...} />` |
| `Highcharts title: null` | Use `title: { text: undefined }` — null breaks Highcharts |
| `Highcharts minorTickInterval: null` | Use `minorTickInterval: undefined` |
...
## 10. Widget `metadata.json` — Required for Every Widget

**Every widget MUST include a `metadata.json` file alongside its component files.** Documents the API surface the widget/connector uses. Generated at creation, updated whenever the data layer changes.
...
### Corrections from real usage — read these before writing metadata.json

| Mistake | Correct |
|---------|---------|
| `"apis": []` — array | `"apis": {}` — **single object**, not an array |
| `"endpoint": "PUT https://..."` — method in URL string | `"method": "PUT"` separate field; `"endpoint"` is path only: `"/api/account/..."` |
| Copied example from skills.md blindly | Document **only** the APIs actually called by this widget/connector — not config panel APIs |
| Vague `responseShape` string | Show exact JS shape + include `"widgetDataProp"` showing exact path to the value |
| Missing `organisation` and `ngsw-bypass` in headers | Always include both |
| No config path references in reqBodySchema | Each field should show where the value comes from: `"config.charts[0].dataConfig.operator"` |
| No `calledFrom` field | Always include `"calledFrom"` — which bundle calls this API and when |
...
```json
{
  "widgetName": "{Name}",
  "version": "1.0.0",
  "description": "Short description of what this widget displays",
  "apis": {
    "id": "getWidgetData",
    "calledFrom": "{Name}Connector — on mount and every refreshInterval tick",
    "endpoint": "/api/account/ioLensWidget/getWidgetData",
    "method": "PUT",
    "headers": {
      "Authorization": "Bearer <token>",
      "Content-Type": "application/json",
    ...
    "reqBodySchema": {
      "startTime": "connectorProps.startTime ?? startOfToday (epoch ms)",
      "endTime": "connectorProps.endTime ?? now (epoch ms)",
      "timezone": "config.time.timezone ?? 'Asia/Calcutta'",
      "timeBucket": ["year", "month", "day", "hour"],
      "timeFrame": "config.time.periodicity ?? 'day'",
      "type": "pieChart",
      "config": [
        {
          "devID": "config.charts[0].dataConfig.devID",
          "sensor": "config.charts[0].dataConfig.sensor",
          "operator": "config.charts[0].dataConfig.operator",
          "key": "config.charts[0].dataConfig.key"
        }
      ]
    },
    "responseShape": { "success": true, "data": [{ "data": "number" }] },
    "widgetDataProp": "data.data[0].data"
  }
}
```

**Note:** If the widget calls multiple APIs, `apis` becomes an array of these objects. If it calls only one API, `apis` is a single object (not wrapped in an array).

---

## 11. Dev Environment Notes

Issues encountered in real sessions — check these before debugging:

| Issue | Fix |
|-------|-----|
| Port 3000 already in use | Use port 4000 for webpack-dev-server |
| Node v14 — webpack-dev-server v5 fails | Use `webpack-dev-server@4` — v5 requires Node 15+ |
| `react-feather` not found | Install it — it's an undeclared peer dep of `@faclon-labs/design-sdk` |
| Highcharts `title: null` throws | Use `title: { text: undefined }` |
| Highcharts `minorTickInterval: null` throws | Use `minorTickInterval: undefined` |
| API request hangs in pending state | Add `'ngsw-bypass': 'true'` header |
| Auth fails with valid token | Add `'organisation': localStorage.getItem('organisation') ?? ''` header |
